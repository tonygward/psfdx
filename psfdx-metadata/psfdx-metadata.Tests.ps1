...
Describe 'Retrieve-SalesforceOrg' {
    BeforeEach {
        Mock -ModuleName 'psfdx-metadata' Invoke-Sf {}
    }

    It 'creates manifest from org and retrieves via manifest' {
        Retrieve-SalesforceOrg -TargetOrg 'user' | Out-Null
        Assert-MockCalled -ModuleName 'psfdx-metadata' Invoke-Sf -Times 1 -ParameterFilter { $Command -like 'sf force source manifest create --from-org user*' }
        Assert-MockCalled -ModuleName 'psfdx-metadata' Invoke-Sf -Times 1 -ParameterFilter { $Command -like 'sf project retrieve start --target-org user*' }
    }
}

Describe 'Retrieve-SalesforceComponent' {
    BeforeEach { Mock -ModuleName 'psfdx-metadata' Invoke-Sf {} }
    ...
    It 'retrieves a named ApexClass for target org' {
        ...
        Assert-MockCalled -ModuleName 'psfdx-metadata' Invoke-Sf -Times 1 -ParameterFilter { $Command -eq 'sf project retrieve start --metadata ApexClass:MyClass --target-org me' }
    }
}
...
Describe 'Get-SalesforceApexClass' {
    BeforeEach {
        ...
        Mock -ModuleName 'psfdx-metadata' Invoke-Sf { $json }
    }
