...
function Watch-SalesforceDebugLogs {
    ...
    $command += @('--color')
    return psfdx-logs\Invoke-Salesforce -Command $command
}
...
function Get-SalesforceDebugLogs {
    ...
    if ($TargetOrg) { $command += @('--target-org', $TargetOrg) }
    $command += @('--json')
    $result = psfdx-logs\Invoke-Salesforce -Command $command
...
function Get-SalesforceDebugLog {
    ...
    if ($TargetOrg) { $command += @('--target-org', $TargetOrg) }
    $command += @('--json')
    $raw = psfdx-logs\Invoke-Salesforce -Command $command
...
function Get-SalesforceLoginHistory {
    ...
    foreach ($record in $records) {
        ...
        $user = $usernames | Where-Object { $_.Id -eq $userId } | Select-Object -First 1
        if (-not $user) {
            continue
        }
...
function Get-SalesforceEventFile {
    ...
    if ($TargetOrg) { $command += " --target-org $TargetOrg" }
    psfdx-logs\Invoke-Salesforce -Command $command
}
